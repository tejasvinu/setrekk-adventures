...
const ToolbarButton = ({ icon: Icon, isActive, onClick, label }: ToolbarButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    className={`p-1 md:p-2 rounded-md hover:bg-slate-800 tooltip-trigger ${
  ...
  return (
    <div className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur-sm border-b border-slate-800">
      <div className="w-full overflow-x-auto">
        <div className="min-w-max max-w-[90%] xl:max-w-8xl mx-auto px-2 md:px-4 py-2 flex flex-wrap gap-2 md:gap-4">
          {EDITOR_GROUPS.map((group) => (
            <div key={group.id} className="flex flex-col">
              <span className="text-[10px] md:text-xs text-slate-500 mb-1">{group.label}</span>
              <div className="flex gap-0.5 md:gap-1 bg-slate-800/50 rounded-md p-0.5 md:p-1">
